import React from "react";
import Logo from "../assets/images/Logo.svg";
import workspace from "../assets/images/Icon/workspace.svg";
import home from "../assets/images/Icon/Home.svg";
import { NavLink } from "react-router-dom";

export default function Sidebar() {
  return (
    <div>
      <div className="h-screen shadow-sm-light bg-[#FAFAF9] w-80 fixed">
            {/* Logo */}
            <div className="flex justify-center ">
              <img src={Logo} alt="" className="w-[100px] h-[100px] " />
            </div>

            {/*  Button New workspace */}
            <div className="flex justify-center m-5 text-18px">
              <button className="rounded py-3 w-full bg-primary hover:bg-primary/90 text-white">
                      New workspace
              </button>
            </div>

          <div className="px-5">
                {/* Home */}
                <div className="bg-[#EFEFEF] mb-3 px-5 py-2.5 rounded-lg">
                    <NavLink to="/dashboard" className={({isActive}) => isActive? "fill-primary text-primary" : "fill-accent text-accent"}>
                          <span className="flex">
                                    <span className="flex pr-4">
                                          <svg width="22" height="22" viewBox="0 0 15 18"  xmlns="http://www.w3.org/2000/svg">
                                          <path d="M14.1667 5.67668L9.16675 1.29335C8.70839 0.883386 8.11502 0.656738 7.50008 0.656738C6.88513 0.656738 6.29176 0.883386 5.83341 1.29335L0.833412 5.67668C0.568719 5.91341 0.357496 6.20381 0.213806 6.52855C0.0701154 6.85329 -0.0027438 7.20492 7.89951e-05 7.56001V14.8433C7.89951e-05 15.5064 0.263471 16.1423 0.732312 16.6111C1.20115 17.08 1.83704 17.3433 2.50008 17.3433H12.5001C13.1631 17.3433 13.799 17.08 14.2678 16.6111C14.7367 16.1423 15.0001 15.5064 15.0001 14.8433V7.55168C15.0017 7.19798 14.9283 6.84796 14.7846 6.52475C14.641 6.20153 14.4304 5.91248 14.1667 5.67668ZM9.16675 15.6767H5.83341V11.51C5.83341 11.289 5.92121 11.077 6.07749 10.9208C6.23377 10.7645 6.44573 10.6767 6.66675 10.6767H8.33341C8.55443 10.6767 8.76639 10.7645 8.92267 10.9208C9.07895 11.077 9.16675 11.289 9.16675 11.51V15.6767ZM13.3334 14.8433C13.3334 15.0644 13.2456 15.2763 13.0893 15.4326C12.9331 15.5889 12.7211 15.6767 12.5001 15.6767H10.8334V11.51C10.8334 10.847 10.57 10.2111 10.1012 9.74225C9.63234 9.27341 8.99645 9.01001 8.33341 9.01001H6.66675C6.0037 9.01001 5.36782 9.27341 4.89898 9.74225C4.43014 10.2111 4.16675 10.847 4.16675 11.51V15.6767H2.50008C2.27907 15.6767 2.0671 15.5889 1.91082 15.4326C1.75454 15.2763 1.66675 15.0644 1.66675 14.8433V7.55168C1.6669 7.43336 1.69224 7.31643 1.74109 7.20866C1.78995 7.1009 1.86119 7.00477 1.95008 6.92668L6.95008 2.55168C7.10215 2.41808 7.29766 2.3444 7.50008 2.3444C7.7025 2.3444 7.898 2.41808 8.05008 2.55168L13.0501 6.92668C13.139 7.00477 13.2102 7.1009 13.2591 7.20866C13.3079 7.31643 13.3333 7.43336 13.3334 7.55168V14.8433Z"/>
                                          </svg>
                                    </span>
                                    <p className="font-bold text-18px">Home</p>
                          </span>
                    </NavLink>
                </div>

                {/* workspace */}
                <div>
                      <div className="bg-[#EFEFEF] mb-5 px-5 py-2.5 rounded-lg">
                            <NavLink to="/workspace" className={({isActive}) => isActive? "fill-primary text-primary" : " fill-accent text-accent"}>
                                <span className="flex">
                                      <span className="pr-4">
                                            <svg width="22" height="22" viewBox="0 0 20 20"  xmlns="http://www.w3.org/2000/svg">
                                          <path d="M8.33333 10.8333H3.33333C3.11232 10.8333 2.90036 10.9211 2.74408 11.0774C2.5878 11.2337 2.5 11.4457 2.5 11.6667V16.6667C2.5 16.8877 2.5878 17.0996 2.74408 17.2559C2.90036 17.4122 3.11232 17.5 3.33333 17.5H8.33333C8.55435 17.5 8.76631 17.4122 8.92259 17.2559C9.07887 17.0996 9.16667 16.8877 9.16667 16.6667V11.6667C9.16667 11.4457 9.07887 11.2337 8.92259 11.0774C8.76631 10.9211 8.55435 10.8333 8.33333 10.8333ZM7.5 15.8333H4.16667V12.5H7.5V15.8333ZM16.6667 2.5H11.6667C11.4457 2.5 11.2337 2.5878 11.0774 2.74408C10.9211 2.90036 10.8333 3.11232 10.8333 3.33333V8.33333C10.8333 8.55435 10.9211 8.76631 11.0774 8.92259C11.2337 9.07887 11.4457 9.16667 11.6667 9.16667H16.6667C16.8877 9.16667 17.0996 9.07887 17.2559 8.92259C17.4122 8.76631 17.5 8.55435 17.5 8.33333V3.33333C17.5 3.11232 17.4122 2.90036 17.2559 2.74408C17.0996 2.5878 16.8877 2.5 16.6667 2.5V2.5ZM15.8333 7.5H12.5V4.16667H15.8333V7.5ZM16.6667 13.3333H15V11.6667C15 11.4457 14.9122 11.2337 14.7559 11.0774C14.5996 10.9211 14.3877 10.8333 14.1667 10.8333C13.9457 10.8333 13.7337 10.9211 13.5774 11.0774C13.4211 11.2337 13.3333 11.4457 13.3333 11.6667V13.3333H11.6667C11.4457 13.3333 11.2337 13.4211 11.0774 13.5774C10.9211 13.7337 10.8333 13.9457 10.8333 14.1667C10.8333 14.3877 10.9211 14.5996 11.0774 14.7559C11.2337 14.9122 11.4457 15 11.6667 15H13.3333V16.6667C13.3333 16.8877 13.4211 17.0996 13.5774 17.2559C13.7337 17.4122 13.9457 17.5 14.1667 17.5C14.3877 17.5 14.5996 17.4122 14.7559 17.2559C14.9122 17.0996 15 16.8877 15 16.6667V15H16.6667C16.8877 15 17.0996 14.9122 17.2559 14.7559C17.4122 14.5996 17.5 14.3877 17.5 14.1667C17.5 13.9457 17.4122 13.7337 17.2559 13.5774C17.0996 13.4211 16.8877 13.3333 16.6667 13.3333ZM8.33333 2.5H3.33333C3.11232 2.5 2.90036 2.5878 2.74408 2.74408C2.5878 2.90036 2.5 3.11232 2.5 3.33333V8.33333C2.5 8.55435 2.5878 8.76631 2.74408 8.92259C2.90036 9.07887 3.11232 9.16667 3.33333 9.16667H8.33333C8.55435 9.16667 8.76631 9.07887 8.92259 8.92259C9.07887 8.76631 9.16667 8.55435 9.16667 8.33333V3.33333C9.16667 3.11232 9.07887 2.90036 8.92259 2.74408C8.76631 2.5878 8.55435 2.5 8.33333 2.5V2.5ZM7.5 7.5H4.16667V4.16667H7.5V7.5Z"/>
                                          </svg>
                                      </span>
                                        <p className="font-bold text-18px">Workspace</p>
                                </span>
                            </NavLink>
                      </div>
                </div>
          </div>
            
      </div>
      
     
    </div>
  );
}
